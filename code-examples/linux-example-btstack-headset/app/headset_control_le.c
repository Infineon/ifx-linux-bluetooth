...
wiced_bt_gatt_status_t hci_control_le_read_by_type_handler (uint16_t conn_id, wiced_bt_gatt_opcode_t opcode,
    wiced_bt_gatt_read_by_type_t *p_read_req, uint16_t len_requested)
{
    attribute_t *puAttribute;
    uint16_t     attr_handle = p_read_req->s_handle;
    uint8_t     *p_rsp = (uint8_t *)wiced_bt_get_buffer(len_requested);
    uint8_t      pair_len = 0;
    int used = 0;
...
wiced_bt_gatt_status_t hci_control_le_gatt_req_cb( wiced_bt_gatt_attribute_request_t *p_req )
{
    ...
    switch ( p_req->opcode)
    {
        ...
        case GATT_REQ_READ_BY_TYPE:
            result = hci_control_le_read_by_type_handler( p_req->conn_id, p_req->opcode, &p_req->data.read_by_type, p_req->len_requested );
            break;
