...
static void bt_hs_spk_handsfree_cb_init_context(void)
{
    ...
    for (i = 0 ; i < WICED_BT_HFP_HF_MAX_CONN ; i++)
    {
        ...
        p_context->audio_config.sr              = AM_PLAYBACK_SR_32K;
#elif (WICED_BT_HFP_HF_WBS_INCLUDED == TRUE)
        ...
        p_context->sco_params.max_latency       = 0x000D;   /* Latency: 13 ms (T2) refer to Table 6.14 of HFP v1.9 */
#else
...
#if (WICED_BT_HFP_HF_SWBS_INCLUDED == TRUE)
        p_context->sco_params.use_wbs           = CODEC_SUPER_WIDE_BAND;
#elif (WICED_BT_HFP_HF_WBS_INCLUDED == TRUE)
        p_context->sco_params.use_wbs           = CODEC_WIDE_BAND;
#else
        p_context->sco_params.use_wbs           = CODEC_NARROW_BAND;
#endif
    }
}
...
#if   (WICED_BT_HFP_HF_SWBS_INCLUDED == TRUE)
            bt_hs_spk_handsfree_cb.context[i].audio_config.sr               = AM_PLAYBACK_SR_32K;
#elif (WICED_BT_HFP_HF_WBS_INCLUDED == TRUE)
...
static void bt_hs_spk_handsfree_sco_management_callback_connected(handsfree_app_state_t *p_ctx, wiced_bt_management_evt_data_t *p_data)
{
    ...
    switch (p_ctx->sco_params.use_wbs)
    {
    case CODEC_SUPER_WIDE_BAND:
        p_ctx->audio_config.sr = AM_PLAYBACK_SR_32K;
        break;
    case CODEC_WIDE_BAND:
        p_ctx->audio_config.sr = AM_PLAYBACK_SR_16K;
        break;
    case CODEC_NARROW_BAND:
...
static void bt_hs_spk_handsfree_event_handler_codec_set(handsfree_app_state_t *p_ctx, wiced_bt_hfp_hf_event_data_t* p_data)
{
    ...
    /* Update information. */
    switch (p_data->selected_codec)
    {
    case WICED_BT_HFP_HF_LC3_CODEC:
        p_ctx->sco_params.use_wbs = CODEC_SUPER_WIDE_BAND;
        p_ctx->audio_config.sr = AM_PLAYBACK_SR_32K;
        p_ctx->sco_params.max_latency = 0x000D;
        break;

    case WICED_BT_HFP_HF_MSBC_CODEC:
        p_ctx->sco_params.use_wbs = CODEC_WIDE_BAND;
        p_ctx->audio_config.sr = AM_PLAYBACK_SR_16K;
        p_ctx->sco_params.max_latency = 0x000D;
        break;

    case WICED_BT_HFP_HF_CVSD_CODEC:
        p_ctx->sco_params.use_wbs = CODEC_NARROW_BAND;
        p_ctx->audio_config.sr = AM_PLAYBACK_SR_8K;
        p_ctx->sco_params.max_latency = 0x000C;
        break;

    default:
        /* Unrecognized codec, do nothing */
        return;
    }
...
wiced_bool_t bt_hs_spk_handsfree_active_call_session_info_get(bt_hs_spk_handsfree_active_call_session_info_t *p_info)
{
    ...
    p_info->wide_band = (wiced_bool_t) bt_hs_spk_handsfree_cb.p_active_context->sco_params.use_wbs;
...
static void bt_hs_spk_handsfree_audio_connection_establish(handsfree_app_state_t *p_ctx)
{
    ...
    if (p_ctx->sco_params.use_wbs != CODEC_NARROW_BAND)
    {
