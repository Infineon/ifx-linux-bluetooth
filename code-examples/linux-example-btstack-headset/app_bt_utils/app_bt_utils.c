...
{
    TRACE_LOG("ADDR:%02X:%02X:%02X:%02X:%02X:%02X", bdadr[0], bdadr[1], bdadr[2], bdadr[3], bdadr[4], bdadr[5]);
}

void trace_log_with_bdaddr(char* string, wiced_bt_device_address_t bdadr){

    TRACE_LOG("%s[%02X:%02X:%02X:%02X:%02X:%02X]",string, bdadr[0], bdadr[1], bdadr[2], bdadr[3], bdadr[4], bdadr[5]);
...
/*******************************************************************************
* Function Name: void sleep_us(unsigned long microseconds)
********************************************************************************
* Summary:
*   This is a utility function that sleep the current thread
*
* Parameters:
...
const char *get_bt_event_name(wiced_bt_management_evt_t event)
{
    switch ((int)event)
    {
        ...
        CASE_RETURN_STR(BTM_BLE_CONNECTION_PARAM_REQUEST_EVENT)
        CASE_DEFAULT_STR
    }
