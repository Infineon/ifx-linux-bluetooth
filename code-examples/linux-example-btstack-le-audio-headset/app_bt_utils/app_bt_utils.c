...
void set_local_bd_addr( void )
{
    ...
    {
        result = wiced_bt_set_local_bdaddr(p, BLE_ADDR_PUBLIC);
        if (result != WICED_SUCCESS)
        {
            TRACE_ERR("set local bda fail:%d", result);
        }
        print_bd_address(p);
    } else {
...
{
    char tmp[4] = {0};
    uint8_t local_dev_length = strlen( (char *)lehs_cfg_settings.device_name ) + sizeof(bt_device_address) + 1;
    uint8_t *local_dev_name = (char*)wiced_memory_allocate(local_dev_length); //don't release, le need use

    memcpy(local_dev_name, lehs_cfg_settings.device_name, strlen(lehs_cfg_settings.device_name) );
