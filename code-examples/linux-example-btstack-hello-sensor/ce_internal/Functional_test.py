...
def hello_sensor_service_handler(logfile, execTimeout):
    '''
    ...
    advert_start = "Advertisement State Change: 4"
    connect_msg = "Connected to Hello Sensor Service"
    port=22
    ...
    time.sleep(10)
    print("Starting Hello Sensor service now...\n")
    file = open(logfile, 'w')
...
def GetLedBlinks(line):
     '''
     This API is used to extract the Led blinks number from the specfic line in the log file.
          
     Input: The log line from which num of blinks set by user need to be extracted.
     
     Output: Num of Blinks
     '''
     start_index = line.rfind(":")
     start_index = start_index+1
     value = int(line[start_index:])
     return value
...
#    device_connected = 0
    hellosensor_log_file = open(logfile, 'r')
    
    hellosensor_logs = hellosensor_log_file.readlines()
    RemoveExtraLine(hellosensor_logs)
 	    
    if (device_connected == 1):
        print("Client and Server connected successfully")
        for i in range(0, len(hellosensor_logs)):
            line = hellosensor_logs[i]
            line = line.replace(" ", "")
            if ("numblinks" in line):
                num_blinks = GetLedBlinks(line)
                print("Number of blinks set to ", num_blinks)
    else:
...
def run_test():
    ...
    logfile = "HelloSensor" + "_" + sys.argv[1] + "_" + sys.argv[2] + hcdtype + ".txt"
    #This thread will handle GATT server commands and logs. Pass log file name in argument
    t1 = threading.Thread(target=hello_sensor_service_handler, args=(logfile,200))
