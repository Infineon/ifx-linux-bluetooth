...
const char *get_bt_event_name(wiced_bt_management_evt_t event)
{
    ...
    CASE_RETURN_STR(BTM_RE_START_EVT)
...
{
    char tmp[4] = {0};
    uint8_t local_dev_length = strlen( (char *)unicast_source_cfg_settings.device_name ) + sizeof(bt_device_address) + 1;
    uint8_t *local_dev_name = (char*)wiced_memory_allocate(local_dev_length); //don't release, le need use

    memcpy(local_dev_name, unicast_source_cfg_settings.device_name, strlen(unicast_source_cfg_settings.device_name) );
