...
{
    char tmp[4] = {0};
    uint8_t local_dev_length = strlen( (char *)lepl_cfg_settings.device_name ) + sizeof(bt_device_address) + 1;
    uint8_t *local_dev_name = (char*)wiced_memory_allocate(local_dev_length); //don't release, le need use

    memcpy(local_dev_name, lepl_cfg_settings.device_name, strlen(lepl_cfg_settings.device_name) );
