 ...
 * @brief AICS init
 *
 * @param[in] num_instances : Number of AICS instances for the application
 * @param[in] pv_ini : expected to be NULL
 */
wiced_result_t wiced_bt_ga_aics_init(uint8_t num_instances, void *pv_ini);

/** Enable AICS server */
void wiced_bt_ga_aics_enable_server(void);

/** Enable AICS client */
void wiced_bt_ga_aics_enable_client(void);
