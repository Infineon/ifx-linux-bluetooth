 ...
 ******************************************************************************
 * Summary:
 *   Function to parse argument when launch MICED BT module.
 *
 * Parameters:
 *   int argc:      counts of argrument.
 *   char **argv:   vectors of argrument.
 *   arg_parser_arguments_t *parsed_args: The structure pointer of parsed argrument data.
 *
 * Return:
 *  int: parse error of success
 *
 *****************************************************************************/
int arg_parser_get_args( int argc,
                        char **argv,
                        arg_parser_arguments_t *parsed_args)
{
    unsigned int len = 0; /* Application name length */
    char local_bda_string[BD_ADDR_LENGTH * 2 + 1]; /* Local BD address string */
    unsigned char r_exist = 0, p_exist = 0;
    cybt_controller_autobaud_config_t *p_autobaud_cfg = &parsed_args->p_gpio_cfg.autobaud_cfg;
    cybt_gpio_wake_on_ble_t *p_wake_on_ble_cfg = &parsed_args->p_gpio_cfg.wake_on_ble_cfg;
    ...
    memset( local_bda_string, 0, sizeof(local_bda_string));
    ...
    for ( int i = 1; i < argc; i++ )
    {
        ...
        case 'C':  /* HCI UART Port */
            if ( argc >= ( i + 2 ) )
            {
                ...
                strncpy( parsed_args->com_port, argv[++i], hci_len );
                parsed_args->com_port[hci_len] = '\0';
            }
            else
            {
        ...
        case 'B':  /* HCI baud rate */
            if ( argc >= ( i + 2 ) )
            {  
                if (EOF == sscanf( argv[++i], "%u", &parsed_args->baud_rate ))
                {
        ...
        case 'F':  /* FW patch download baud rate */
            if ( argc >= ( i + 2 ) )
            {
                if (EOF == sscanf( argv[++i], "%u", &parsed_args->patch_baud ))
                {
                    TRACE_ERR( "set baud rate error in Command line options.\n" );
                    print_usage( app_name );
                    return PARSE_ERROR;
        ...
        case 'P':  /* FW patch file to download */
            if ( argc >= ( i + 2 ) )
            {
                ...
                strncpy( parsed_args->patchFile, argv[++i], len );
                parsed_args->patchFile[len] = '\0';
                p_exist = 1;
            }
            else
            {
        ...
        case 'I':  /* SPY Instance */
            {
                int spy_ins = atoi( argv[++i] );
                if ( spy_ins < 0 )
                {
                    parsed_args->spy_inst = 0;
                }
                else
				{
                    parsed_args->spy_inst = spy_ins;
                }
            break;
            }
        ...
        case 'D':  /* Device BD Address */
            if ( argc >= ( i + 2 ) )
            {
                ...
                AsciiToHex( bda, BD_ADDR_LENGTH, local_bda_string );
                stream_to_bdaddr( parsed_args->local_bda, bda );
            }
            else
            {
        ...
        case 'T':  /* Peer IP address for BYSPY connection */
            if ( argc >= ( i + 2 ) )
            {
                ...
                strncpy( parsed_args->peer_ip_addr, argv[++i], len);
				parsed_args->peer_ip_addr[len] = '\0';
            }
            else
            {
                /* Set default to localHost */
                strcpy( parsed_args->peer_ip_addr, "127.0.0.1" );
            }
            break;

        case 'S':  /* TCP Socket for BTSPY connection */
            parsed_args->is_socket_tcp = TRUE;
            break;
        case 'N':  /* use ioctl */
            p_autobaud_cfg->use_ioctl = TRUE;
            break;
