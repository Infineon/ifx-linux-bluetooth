...
extern sem_t uart_recv_semaphore;
...
BOOL32 uartInit (char *pPortName, uint32_t uBaudRate)
{
    ...
    // clear uart buffer, input and ouput
    if (tcflush(userial_cb.fd, TCIOFLUSH) != 0)
    {
        perror("Failed to flush UART buffer");
        close(userial_cb.fd);
        exit(1);
    }
...
static void rx_enqueue(void)
{
    ...
    memcpy(p_rx_pkt, &m_rx_pkt, sizeof(tUART_RX));

    wiced_bt_enqueue(&rx_queue, p_rx_pkt);

    memset (&m_rx_pkt, 0, sizeof (m_rx_pkt));

    m_rx_pkt.state = WAIT_TYPE;
...
void*  uartRecv (void *ppp)
{
    int	dwBytesRead = 0;
    uint16_t i      = 0;
    uint8_t  scratchPad[2000] = {0};

    sem_post(&uart_recv_semaphore);
    ...
    while (userial_cb.fd != -1)
    {
        ...
        if (dwBytesRead)
        {
            ...
            {
                int to_copy = 1; // process only 1 byte by default, unless in WAIT_DATA state
                if(m_rx_pkt.state == WAIT_DATA)
                {
                    to_copy = m_rx_pkt.remaining_hci_len; // remaining_length to copy
                    if(to_copy > (dwBytesRead - i))
                    {
                        to_copy = dwBytesRead - i;
                    }
                    if(to_copy + m_rx_pkt.len > sizeof(m_rx_pkt.data)){
                        TRACE_ERR("bad receive size %d exceeds receive buffer %d",to_copy + m_rx_pkt.len, sizeof(m_rx_pkt.data));
                        break;
                    }
                    memcpy( &m_rx_pkt.data[m_rx_pkt.len], &scratchPad[i], to_copy);
                    m_rx_pkt.len += to_copy;
                    m_rx_pkt.remaining_hci_len -= to_copy;
                }
                else
                {
                ...
                i+= to_copy;
                if ((m_rx_pkt.state == WAIT_DATA) && (m_rx_pkt.remaining_hci_len == 0))
                {
                    pktRcvd();
                }
            }
...
**  Processes byte at a time, returns immediately in case data is processed
*/
static wiced_bool_t ProcessChar(uint8_t in_byte)
{
    switch (m_rx_pkt.state)
    {
      case WAIT_TYPE:
        m_rx_pkt.len = m_rx_pkt.remaining_hci_len = 0; // new packet
        switch (in_byte)
        {
          case HCIT_TYPE_ACL_DATA:
            m_rx_pkt.state = WAIT_HANDLE_1; // read 2 bytes for the handle
            m_rx_pkt.type  = HCIT_TYPE_ACL_DATA;
            break;

          case HCIT_TYPE_SCO_DATA:
            m_rx_pkt.state = WAIT_HANDLE_1;
            m_rx_pkt.type  = HCIT_TYPE_SCO_DATA;
            break;

          case HCIT_TYPE_EVENT:
            m_rx_pkt.state = WAIT_OPCODE_1;
            m_rx_pkt.type  = HCIT_TYPE_EVENT;
            break;

          case HCIT_TYPE_WICED_HCI:
              m_rx_pkt.state = WAIT_HANDLE_1;
              m_rx_pkt.type  = HCIT_TYPE_WICED_HCI;
              break;

          case HCIT_TYPE_DIAG:
              m_rx_pkt.state = WAIT_DATA;
              m_rx_pkt.remaining_hci_len = 63;
              m_rx_pkt.type  = HCIT_TYPE_DIAG;
              break;

          case HCIT_TYPE_ISOC_DATA:
              m_rx_pkt.state = WAIT_HANDLE_1;
              m_rx_pkt.type  = HCIT_TYPE_ISOC_DATA;
              break;
          ...
          default:
            ...
            flush_debug_rx();
#endif
            TRACE_ERR ("[linux] ProcessChar: bad HCI H4 type %d", in_byte);
      ...
      case WAIT_OPCODE_1:
        m_rx_pkt.data[m_rx_pkt.len++] = in_byte;
        m_rx_pkt.state = WAIT_LEN_1_BYTE;
        break;

      case WAIT_HANDLE_1:
        m_rx_pkt.data[m_rx_pkt.len++] = in_byte;
        m_rx_pkt.state = WAIT_HANDLE_2;
        break;

      case WAIT_HANDLE_2:
        m_rx_pkt.data[m_rx_pkt.len++] = in_byte;
        m_rx_pkt.state = WAIT_LEN_1;
        break;

      case WAIT_LEN_1_BYTE:
        m_rx_pkt.data[m_rx_pkt.len++] = in_byte;
        m_rx_pkt.remaining_hci_len = in_byte;
        m_rx_pkt.state = WAIT_DATA;
        break;
      ...
      case WAIT_LEN_1:
        m_rx_pkt.data[m_rx_pkt.len++] = in_byte;
        m_rx_pkt.remaining_hci_len = in_byte;
        m_rx_pkt.state = WAIT_LEN_2;
        break;

      case WAIT_LEN_2:
        m_rx_pkt.data[m_rx_pkt.len++] = in_byte;
        m_rx_pkt.remaining_hci_len += (in_byte << 8);
        m_rx_pkt.state = WAIT_DATA;

        if (m_rx_pkt.type == HCIT_TYPE_ISOC_DATA)
        {
            m_rx_pkt.remaining_hci_len &= 0x3FFF; // 14 bit for data length, 2 bit RFU
        }

        if (m_rx_pkt.remaining_hci_len == 0)
            pktRcvd();
        else
            m_rx_pkt.state = WAIT_DATA;
        break;
      ...
      case WAIT_DATA:
        ...
        break;
      default:
        TRACE_ERR("unknow m_rx_pkt.state:%d", m_rx_pkt.state);
        return FALSE;
...
static void pktRcvd (void)
{
    ...
    m_rx_pkt.state = WAIT_TYPE;
    // We only care about event packets
    if (m_rx_pkt.type == HCIT_TYPE_EVENT)
    {
        ProcessEventFromHCI (m_rx_pkt.data, m_rx_pkt.len);
    }
    else if (m_rx_pkt.type == HCIT_TYPE_ACL_DATA)
    {
        ProcessAclFromHCI (m_rx_pkt.data, m_rx_pkt.len);
    }
    else if (m_rx_pkt.type == HCIT_TYPE_DIAG)
    {
        ProcessDiagFromHCI(m_rx_pkt.data, m_rx_pkt.len);
    }
    else if (m_rx_pkt.type == HCIT_TYPE_SCO_DATA)
    {
        ProcessScoFromHCI(m_rx_pkt.data, m_rx_pkt.len);
    }
    else if (m_rx_pkt.type == HCIT_TYPE_ISOC_DATA)
    {
        ProcessIsocFromHCI(m_rx_pkt.data, m_rx_pkt.len);
    }
    else if (m_rx_pkt.type == HCIT_TYPE_WICED_HCI)
    {
        TRACE_LOG("!!!!Dropping WICED HCI Packet Length: %u\n", m_rx_pkt.len);
    }
    else
    {
        TRACE_ERR("!!!!Unexpected HCI H4 Packet Type: %u  Length: %u\n", m_rx_pkt.type, m_rx_pkt.len);
    }
    //memset (m_rx_pkt.data, 0, sizeof (m_rx_pkt.data));
}
