...
sem_t timer_task_semaphore;
sem_t uart_recv_semaphore;
...
#define SPY_TRACE_TYPE_CRIT_EVENT   40

#define GPIO_ACTIVE_HIGH      1
#define GPIO_ACTIVE_LOW       0

#if (defined(BTSTACK_VER) && (BTSTACK_VER > 0x03090000))
pf_wiced_exception pf_linux_exception = NULL;
#endif
...
BOOL32 cy_platform_bluetooth_init(char* patchFile, char *device, uint32_t baudrate, uint32_t fw_download_baudrate, const cybt_controller_autobaud_config_t *p_autobaud_cfg)
{
    ...
    sem_init(&timer_task_semaphore, 0, 1);
    sem_init(&uart_recv_semaphore, 0, 1);
    ...
    //wait uart recv thread ready
    sem_wait(&uart_recv_semaphore);

    if (baudrate != 0)
    {
        app_baud_rate = checkAppBaudRate(baudrate);
        if (patch_downloaded == TRUE)
        { //if not download FW, change app baudrate.
...
void* timerTask(void *t)
{
    uint32_t last_tc, cur_tc = 0, sleepTime = 0;
    uint64_t curAbsTime      = 0;
...
// Debug traces
//
static void SendTextTraceToSpy(char *str, int length, uint8_t spy_instance, BYTE trace_type)
{
    char buff[1000];
    char *p;
    int x;
    ...
    printf("%s %s\n",buff, str); // print onto console
    TraceHciPkt(spy_trace_types[trace_type], (BYTE *)str, length, spy_instance);
}

void emu_Trace(char *traceBuf, int length, wiced_bt_trace_type_t trace_type)
{
    SendTextTraceToSpy(traceBuf, length, wicedx_emulator_instance, trace_type);
}
...
void debug_Printf(char *format, ...)
{
    ...
    SendTextTraceToSpy(buff, strlen(buff), wicedx_emulator_instance, FALSE);
}
...
void debug_PrintError(char *format, ...)
{
    ...
    SendTextTraceToSpy(buff, strlen(buff), wicedx_emulator_instance, TRUE);
}
...
static void linux_stack_exception(uint16_t code, void* ptr, uint32_t length)
{
#if (defined(BTSTACK_VER) && (BTSTACK_VER > 0x03090000))
    if(pf_linux_exception!=NULL)
    {
        pf_linux_exception(code, (uint8_t *)ptr, length);
    }
    else
    {
	char buf[50]={0};
	...
	while(ptr_temp && len--)
	{
        ...
        TRACE_ERR("0x%x %s len:%lu reason:\"%s\"\n", code, msg, length, buf);
    }
#endif
    // abort();
}
...
void wiced_bt_platform_interface_init(void)
{
    ...
    platform_intf.pf_get_trng           = NULL;
