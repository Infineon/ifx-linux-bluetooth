...
/******************************************************************************
*
* Function Name: wiced_bt_a2dp_sbc_cfg_for_cap
*
* \brief Determine the preferred SBC codec configuration for the given codec capabilities.
*
                                      ...
                                      wiced_bt_a2d_sbc_cie_t *output_cap)
{
    ...
    else
    {
        ...
        else
        {
            ...
            goto wiced_bt_a2dp_sbc_parse_error;
        }
    }
...
wiced_bt_a2dp_sbc_parse_error:
    if (status == 0)
    {
        memcpy(output_cap, &peer_cie, sizeof(wiced_bt_a2d_sbc_cie_t));
    }
    return status;
}
...
/******************************************************************************
*
* Function Name: wiced_bt_a2dp_sbc_cfg_in_cap
*
...
{
    return ((cap_1->samp_freq == cap_2->samp_freq) && (cap_1->ch_mode == cap_2->ch_mode) &&
            (cap_1->block_len == cap_2->block_len) && (cap_1->num_subbands == cap_2->num_subbands) &&
            (cap_1->alloc_mthd == cap_2->alloc_mthd) && (cap_1->max_bitpool == cap_2->max_bitpool) &&
            (cap_1->min_bitpool == cap_2->min_bitpool));
}
