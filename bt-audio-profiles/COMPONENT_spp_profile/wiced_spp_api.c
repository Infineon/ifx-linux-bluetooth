...
static wiced_bt_uuid_t spp_uuid = {
    .len = 2,
    .uu = {.uuid16 = UUID_SERVCLASS_SERIAL_PORT},
};
...
void spp_rfcomm_closed(spp_scb_t *p_scb)
{
    ...
    if ((p_scb->state == SPP_SESSION_STATE_OPENING) ||
        (!p_scb->b_is_initiator && (p_scb->state == SPP_SESSION_STATE_IDLE)))
    {
        if (p_scb->p_spp_reg->p_connection_failed_callback) {
            p_scb->p_spp_reg->p_connection_failed_callback();
        }
...
wiced_bool_t spp_sdp_find_attr(spp_scb_t *p_scb)
{
    ...
    p_rec = wiced_bt_sdp_find_service_uuid_in_db((wiced_bt_sdp_discovery_db_t *)p_scb->p_sdp_discovery_db,
                                                 &spp_uuid,
                                                 p_rec);
    if (p_rec == NULL) {
...
void spp_sdp_start_discovery(spp_scb_t *p_scb)
{
    ...
    result = wiced_bt_sdp_init_discovery_db((wiced_bt_sdp_discovery_db_t *)p_scb->p_sdp_discovery_db,
                                            1024,
                                            1,
                                            &spp_uuid,
                                            num_attr,
    ...
    if ((result == WICED_FALSE) ||
        (!wiced_bt_sdp_service_search_attribute_request(p_scb->server_addr,
                                                        (wiced_bt_sdp_discovery_db_t *)p_scb->p_sdp_discovery_db,
                                                        &spp_sdp_cback)))
    {
